...
const FolderSideBar = ({ data }) => {
  ...
  const getAllNotesByFolder = async (folderId) => {
    const api = `https://reddy-client-33.onrender.com/notes/folder/${folderId}`;
  ...
  try {
    ...
    const response = await fetch(`https://reddy-client-33.onrender.com/folders`, options);
...
const deleteFolder = async (folderId) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      await axios.delete(`https://reddy-client-33.onrender.com/folders/${folderId}`);
      window.location.reload()
